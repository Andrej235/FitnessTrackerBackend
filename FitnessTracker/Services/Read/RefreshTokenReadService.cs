...
namespace FitnessTracker.Services.Read
{
    public class RefreshTokenReadService(ExerciseContext context) : IReadService<RefreshToken>
    {
        ...
        public Task<List<RefreshToken>> Get(Expression<Func<RefreshToken, bool>> criteria, int? offset = 0, int? limit = -1, string? include = "all") => Task.FromResult<List<RefreshToken>>([]);
